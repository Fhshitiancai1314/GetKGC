...
class BaseDataset(Dataset):
    ...
    def construct_input_tail_text(self, src_ent=None):
        src_name = self.ent_names[src_ent]
        src_desc = ':' + self.ent_descs[src_ent] if self.configs.desc_max_length > 0 else ''
    ...
    def collate_fn(self, data):
        ...
        agg_data['tail_source_ids'] = dataloader_output_to_tensor(data, 'tail_source_ids', padding_value=0)
        agg_data['tail_source_mask'] = dataloader_output_to_tensor(data, 'tail_source_mask', padding_value=0)
...
# 对于训练集采取*2操作，同时训练预测头部和预测尾部，通过index判断
class CELossDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if mode == 'predict_tail':
            ...
            tail_src = self.construct_input_tail_text(src_ent=tail)
            tgt_ent = tail
            ...
            tail_src = self.construct_input_tail_text(src_ent=head)
            tgt_ent = head
        else:
            raise ValueError('Mode is not correct!')
        ...
        tokenized_src = self.tok(tail_src, text_pair='', max_length=self.configs.text_len, truncation=True)
        tail_source_ids = tokenized_src.input_ids
        tail_source_mask = tokenized_src.attention_mask
        if self.mode == 'train' and self.configs.n_lar > 0:
        ...
        out = {
            ...
            'tail_source_ids': tail_source_ids,
            'tail_source_mask': tail_source_mask,
            'triple': triple,
            'ent_rel': ent_rel,
            'tgt_ent': tgt_ent
        }
